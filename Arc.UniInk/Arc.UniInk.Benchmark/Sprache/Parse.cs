namespace Sprache
{
    ...
    /// <summary>Parsers and combinators.</summary>
    public static partial class Parse
    {
        /// <summary>Message for a failure result when left recursion is detected.</summary>
        public const string LeftRecursionErrorMessage = "Left recursion in the grammar.";

        /// <summary>TryParse a single character matching 'predicate'</summary>
        ...
        /// <summary>Parse a single character c.</summary>
        public static Parser<char> Char(char c) => Char(ch => c == ch, char.ToString(c));
        
        /// <summary>Parse a single character of any in c</summary>
        public static Parser<char> Chars(params char[] c) => Char(c.Contains, string.Join("|", c));

        /// <summary>Parse a single character of any in c</summary>
        public static Parser<char> Chars(string c) => Char(c.Contains, string.Join("|", c));
        ...
        /// <param name="c"></param>
        /// <returns></returns>
        public static Parser<char> CharExcept(string c) => CharExcept(c.Contains, string.Join("|", c));
        ...
        public static Parser<IEnumerable<char>> IgnoreCase(string s)
        {
            ...
            return s.Select(IgnoreCase).Aggregate(Return(Enumerable.Empty<char>()), (a, p) => a.Concat(p.Once())).Named(s);
        }
        ...
        public static Parser<IEnumerable<char>> String(string s)
        {
            ...
            return s.Select(Char).Aggregate(Return(Enumerable.Empty<char>()), (a, p) => a.Concat(p.Once())).Named(s);
        }
        ...
        public static Parser<T> End <T>(this Parser<T> parser)
        {
            ...
            return i => parser(i).IfSuccess(s => s.Remainder.AtEnd ? s : Result.Failure<T>(s.Remainder, $"unexpected '{s.Remainder.Current}'", new[] { "end of input" }));
        }
        ...
        public static Parser<T> Ref <T>(Func<Parser<T>> reference)
        {
            ...
            return i =>
            {
                ...
                if (i.Memos.ContainsKey(p))
                {
                    ...
                    if (!pResult.WasSuccessful && pResult.Message == LeftRecursionErrorMessage) throw new ParseException(pResult.ToString());
                }
        ...
        private static IResult<T> DetermineBestError <T>(IResult<T> firstFailure, IResult<T> secondFailure)
        {
            ...
            if (secondFailure.Remainder.Position == firstFailure.Remainder.Position) return Result.Failure<T>(firstFailure.Remainder, firstFailure.Message, firstFailure.Expectations.Union(secondFailure.Expectations));
        ...
        public static Parser<T> Where <T>(this Parser<T> parser, Func<T, bool> predicate)
        {
            ...
            return i => parser(i).IfSuccess(s => predicate(s.Value) ? s : Result.Failure<T>(i, $"Unexpected {s.Value}.", Array.Empty<string>()));
        }
